...
async def root(request: Request):
    ...
    return templates.TemplateResponse("index.j2",
                                      {"request": request, "fasta": db_fasta, "nj_tree": db_nj_tree, "sra_list": db_sra_list,
                                       "ml_tree": db_ml_tree})
...
# @test.get("/mtbc_ml_tree_from_db/{id}")
# async def nj_tree_from_fasta(id: str = ""):
#     try:
#         client = MongoClient('mongodb://{0}:{1}/'.format(mongosettings.host, mongosettings.port))
#         db_mtbc = client.db_mtbc
#         request_data = db_mtbc.request_data
...
#         return resultat
#     else:
#         ml_tree = mtbc_tools.MtbcTree.create_ml_tree_static(id)
#         try:
#             client = MongoClient('mongodb://localhost:27017/')
#             db_mtbc = client.db_mtbc
#             request_data = db_mtbc.request_data
...
# async def ml_tree_from_fasta(id: str = ""):
#     try:
#         client = MongoClient('mongodb://{0}:{1}/'.format(mongosettings.host, mongosettings.port))
#         db_mtbc = client.db_mtbc
#         request_data = db_mtbc.request_data
...
#     resultat_obj = json.loads(resultat_json, object_hook=lambda d: SimpleNamespace(**d))
#     logger.info(resultat_obj._id)
#     ml_tree = mtbc_tools.MtbcTree(resultat_obj)
#     ml_tree.create_ml_tree()
#     return FileResponse(path='ml_tree/{0}'.format(ml_tree._id), media_type='text/plain',
#                         filename="{0}-ml.nwk".format(ml_tree._id))
...
#     logger.info(get_id)
#     client.close()
#     return FileResponse(path='nj_tree/{0}-nj'.format(mtbc_fasta._id), media_type='text/plain',
#                         filename="{0}.nwk".format(mtbc_fasta._id))
#
#
...
#     client.close()
#
#     return FileResponse(path='ml_tree/{0}'.format(mtbc_fasta._id), media_type='text/plain',
#                         filename="{0}-ml.nwk".format(mtbc_fasta._id))
...
#     logger.info(get_id)
#     client.close()
#     return FileResponse(path='alignement/{0}'.format(mtbc_fasta._id), media_type='text/plain',
#                         filename="{0}.fasta".format(mtbc_fasta._id))
...
#     resultat_obj = json.loads(resultat_json, object_hook=lambda d: SimpleNamespace(**d))
#     logger.info(resultat_obj._id)
#     nj_tree = mtbc_tools.MtbcTree(resultat_obj)
#     nj_tree.create_nj_tree()
#     return FileResponse(path='nj_tree/{0}'.format(nj_tree._id), media_type='text/plain',
#                         filename="{0}-nj.nwk".format(nj_tree._id))
